...
import { Methods, PathHandler, RequestHandler, RouteHandler } from "./types";

type MethodName = Exclude<keyof Methods, "use">;
...
export class MiniRouter {
  ...
  // Overload types
  public use(pathMatch: string, mRouter: MiniRouter | PathHandler): void;

  public use(mRouter: PathHandler | MiniRouter): void;

  public use(
    pathMatch: string | PathHandler | MiniRouter,
    mRouter?: MiniRouter | PathHandler
  ): void {
    const keys: RegexKey[] = [];
    ...
    if (mRouter instanceof MiniRouter) {
      ...
      throw new Error("You can only use a MiniRouter or a function");
    }
    ...
    this._methods.use
      ...
      .forEach(tHandler => {
        ...
        if (tUseMatches) {
          ...
          const useHandlers = tHandler.router.processRequest(
